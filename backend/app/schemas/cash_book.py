...
class CashBookItem(BaseModel):
    ...
    transaction_type: str = Field(
        ...,
        description="Type: opening_balance, cash_sale, collection, expense, cash_out",
    )
...
class CashBookSummary(BaseModel):
    ...
    today_cash_out: float = Field(0.0, description="Cash out withdrawals during period")
    total_cash_out: float = Field(0.0, description="Cash out withdrawals during period")
    ...
    total_cash_paid: float = Field(..., description="Total cash paid in period (all outflows: expense + cash out)")

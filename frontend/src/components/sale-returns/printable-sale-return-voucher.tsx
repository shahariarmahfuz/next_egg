...
export interface PrintableSaleReturnVoucherProps {
  saleReturn: SaleReturnItem;
}

export const PrintableSaleReturnVoucher = React.forwardRef<HTMLDivElement, PrintableSaleReturnVoucherProps>(
  ({ saleReturn: ret }, ref) => {
    const filters = [
      { label: "Customer Name", value: ret.customer?.name || "N/A" },
      { label: "Customer Code", value: ret.customer?.customer_code || "N/A" },
      { label: "Return Date", value: formatDateTime(ret.return_date) },
      { label: "Ref Sales Invoice", value: ret.sale?.invoice_no || "N/A" },
    ];
        ...
        <div className="w-80 border border-slate-400 bg-slate-50 text-[11px] rounded overflow-hidden">
          ...
          <div className="p-3 space-y-1.5">
            <div className="flex justify-between">
              <span className="font-semibold text-slate-700">Gross Return Value:</span>
              <span className="font-bold text-slate-900">{formatCurrency(ret.grand_total)}</span>
            </div>
            <div className="flex justify-between">
              <span className="font-semibold text-slate-700">Cash Refund Paid:</span>
              <span className="text-slate-800">{formatCurrency(ret.refund_amount)}</span>
            </div>
            <div className="flex justify-between border-t border-slate-400 pt-1.5 text-xs">
              <span className="font-extrabold text-amber-900">Credit Adjusted on Due:</span>
              <span className="font-extrabold text-amber-950">
                {formatCurrency(ret.grand_total - ret.refund_amount)}
              </span>
            </div>
          </div>
        </div>
    ...
    return (
      <ReportPrintLayout
        ref={ref}
        title="OFFICIAL SALE RETURN VOUCHER"
        subtitle="Credit Note & Customer Return Record"
        documentNo={ret.return_no}
        filters={filters}
        totals={totalsBlock}
        showSignatures
        signatureTitles={["Customer Signature", "Authorized Inspector"]}
      >
...
PrintableSaleReturnVoucher.displayName = "PrintableSaleReturnVoucher";

              ...
              <div className="pl-9 space-y-1 animate-in fade-in-50">
                ...
                {hasPermission(["farm.report", "farm.view"]) && (
                  <Link
                    ...
                    className={cn(
                      "flex items-center space-x-2 px-3 py-2 rounded-lg text-xs font-medium transition-colors",
                      pathname === "/farm/ledger" ? "bg-primary/15 text-primary font-semibold" : "text-muted-foreground hover:text-foreground hover:bg-accent/40"
                    )}
                  >
...
export function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        "hidden md:flex flex-col border-r border-border bg-card transition-all duration-300 relative z-30 h-full md:h-[100dvh] shrink-0 sticky top-0 text-card-foreground print:hidden",
        collapsed ? "w-20" : "w-64"
      )}
    >

...
import { FarmBalanceItem, FarmDeliveryItem, FarmDeliveryUpdatePayload } from "@/types";
...
export default function ManageDeliveryPage() {
  ...
  // Modals state
  const [viewingItem, setViewingItem] = useState<FarmDeliveryItem | null>(null);
  const [editingItem, setEditingItem] = useState<FarmDeliveryItem | null>(null);
  const [deletingItem, setDeletingItem] = useState<FarmDeliveryItem | null>(null);
  ...
  // Query: Paginated delivery records
  const {
    data: responseData,
    isLoading,
    isFetching,
    ...
    queryKey: ["farm-delivery-manage", page, pageSize, farmId, startDate, endDate, search],
    queryFn: () =>
    ...
    enabled: hasPermission(["farm.delivery.view", "delivery.view"]),
  });

  const deliveries: FarmDeliveryItem[] = responseData?.data?.items || [];
  const totalItems = responseData?.data?.total || 0;
  const totalPages = responseData?.data?.pages || Math.ceil(totalItems / pageSize) || 1;

  // Mutation: Update delivery record
  const updateMutation = useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: FarmDeliveryUpdatePayload }) =>
      farmService.updateDelivery(id, payload),
    onSuccess: () => {
      toast.success("Delivery record updated successfully.");
      setEditingItem(null);
      queryClient.invalidateQueries({ queryKey: ["farm-delivery-manage"] });
      queryClient.invalidateQueries({ queryKey: ["farms-list"] });
    },
    onError: (err: any) => {
      const msg = err?.response?.data?.message || err?.message || "Failed to update delivery record.";
      setEditError(msg);
      toast.error(msg);
    },
  });

  // Mutation: Delete delivery record
  const deleteMutation = useMutation({
    mutationFn: (id: string) => farmService.deleteDelivery(id),
    onSuccess: () => {
    ...
    onError: (err: any) => {
      const msg = err?.response?.data?.message || err?.message || "Failed to delete delivery record.";
      toast.error(msg);
    },
  });

  const openEditModal = (item: FarmDeliveryItem) => {
    setEditingItem(item);
    setEditFarmId(item.farm_id);
    setEditDestination(item.destination || "");
    setEditDate(item.delivery_date ? item.delivery_date.split("T")[0] : "");
  ...
  const handleSaveEdit = (e: React.FormEvent) => {
    ...
    const trimmedDest = editDestination.trim();
    if (!trimmedDest) {
      setEditError("Destination / Store name is required.");
      return;
    }

    const qty = parseFloat(sanitizeNumericInput(editTrayQuantity));
    if (isNaN(qty) || qty <= 0) {
      setEditError("Delivery tray quantity must be greater than 0.");
      return;
    }
  ...
  const totalTraysOnPage = deliveries.reduce((sum, item) => sum + (Number(item.tray_quantity) || 0), 0);

  return (
    <HasPermission
      code={["farm.delivery.view", "delivery.view"]}
      fallback={
        <div className="p-8 text-center text-destructive font-medium">
          Access Denied: You do not have permission to view Delivery Management.
        </div>
      }
    >
      <div className="space-y-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <PageHeader
            ...
            description="Audit dispatched trays, search delivery destinations, edit transfer logs, and monitor distribution."
          />
        ...
        <Card className="glass-card shadow-sm border">
          <CardContent className="p-4 space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-12 gap-3">
              ...
              {/* Search text (destination / notes / farm) */}
              <div className="sm:col-span-1 md:col-span-3 relative">
                <Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" />
                <Input
                  placeholder="Search destination, notes, farm..."
                  value={search}
                  onChange={(e) => {
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t text-xs text-muted-foreground">
              <div className="flex items-center gap-2">
                ...
                <Badge variant="outline" className="font-medium bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20">
                  Page Delivered: {totalTraysOnPage.toLocaleString(undefined, { minimumFractionDigits: 1, maximumFractionDigits: 1 })} trays
                </Badge>
              </div>
              {hasActiveFilters && (
              ...
              <table className="w-full text-xs text-left">
                <thead className="bg-muted/50 border-b text-muted-foreground uppercase text-[11px] font-semibold tracking-wider">
                  <tr>
                    ...
                    <th className="py-3 px-4 text-right">Delivered (Trays)</th>
                    ...
                    <tr>
                      <td colSpan={7} className="py-12 text-center text-muted-foreground">
                        <div className="flex flex-col items-center justify-center gap-2">
                          ...
                          <span className="text-xs">Loading delivery records...</span>
                        </div>
                      </td>
                    </tr>
                  ) : deliveries.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="py-12 text-center text-muted-foreground">
                        <div className="flex flex-col items-center justify-center gap-2">
                          ...
                          <span className="text-sm font-medium">No delivery records found</span>
                          <span className="text-xs text-muted-foreground">
                            {hasActiveFilters
                              ? "Try adjusting your filters or search terms."
                              : "Get started by recording farm deliveries."}
                          </span>
                        </div>
                      </td>
                    </tr>
                        ...
                        >
                          ...
                          <td className="py-3 px-4">
                            <div className="flex items-center gap-1.5 font-medium text-foreground">
                              ...
                              <span className="truncate max-w-[150px]">{farmDisplay}</span>
                            </div>
                          </td>
                          ...
                          <td className="py-3 px-4">
                            <div className="flex items-center gap-1.5 font-semibold text-foreground">
                              ...
                              <span className="truncate max-w-[170px]">{item.destination}</span>
                            </div>
                          </td>
                          ...
                          <td className="py-3 px-4 text-center">
                            <div className="flex items-center justify-center gap-1">
                              ...
                              {/* Edit Action */}
                              {hasPermission(["farm.delivery.edit", "delivery.edit"]) && (
                                <Button
                                  variant="ghost"
                                  size="icon"
                              ...
                              {/* Delete Action */}
                              {hasPermission(["farm.delivery.delete", "delivery.delete"]) && (
                                <Button
                                  variant="ghost"
                                  size="icon"
          ...
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-base">
                ...
                <span>Delivery Record Details</span>
              </DialogTitle>
              <DialogDescription className="text-xs">
                Detailed delivery record and destination transfer audit.
              </DialogDescription>
            </DialogHeader>
            ...
            {viewingItem && (
              <div className="space-y-3 py-2 text-xs">
                <div className="grid grid-cols-2 gap-2 p-3 rounded-lg bg-muted/40 border">
                  ...
                  <div>
                    <span className="text-muted-foreground block text-[11px]">Delivered Quantity</span>
                ...
                <div className="space-y-2 p-3 rounded-lg border">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-semibold text-foreground">{viewingItem.destination}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-muted-foreground">Delivery Date</span>
                    <span className="font-medium">{formatDate(viewingItem.delivery_date)}</span>
                  </div>
                  <div className="flex items-center justify-between">
          ...
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="text-base flex items-center gap-2">
                ...
                <span>Edit Delivery Record</span>
              </DialogTitle>
              <DialogDescription className="text-xs">
                Update delivery details or quantity. Available farm tray balance will be checked automatically.
              </DialogDescription>
            </DialogHeader>
            ...
            <form onSubmit={handleSaveEdit} className="space-y-4 py-2">
              ...
              <div className="space-y-1.5">
                <Label className="text-xs">Destination / Store Point</Label>
                <Input
                  type="text"
                  placeholder="e.g. Ayonal, Shop, Central Storage"
              ...
              {/* Tray Quantity */}
              <div className="space-y-1.5">
                <Label className="text-xs">Delivered Tray Quantity</Label>
                <Input
                  type="number"
                  step="any"
                  min="0.1"
              ...
              <div className="space-y-1.5">
                ...
                <Textarea
                  placeholder="Delivery remarks or vehicle details..."
                  value={editNotes}
          ...
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription className="text-xs text-muted-foreground pt-2">
                Are you sure you want to delete this delivery of{" "}
                <strong className="text-foreground font-semibold">
                  {deletingItem?.tray_quantity} trays
                </strong>{" "}
                to{" "}
                <strong className="text-foreground font-semibold">{deletingItem?.destination}</strong>{" "}
                from{" "}
                <strong className="text-foreground font-semibold">
                ...
                on{" "}
                <strong className="text-foreground font-semibold">
                  {deletingItem?.delivery_date ? formatDate(deletingItem.delivery_date) : ""}
                </strong>
                ?
                <br />
                <br />
                Deleting this delivery will return the delivered trays to the farm's available stock.
              </DialogDescription>
            </DialogHeader>

...
export default function FarmWastePage() {
  ...
  const fetchHistory = useCallback(async () => {
    setLoading(true);
    try {
      const res = await farmService.getWasteHistory({
        page,
        size: pageSize,
      });
    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border pb-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-foreground">Record Farm Waste / Loss</h1>
            <p className="text-xs sm:text-sm text-muted-foreground">
              Log broken, cracked, spoiled, or expired items to adjust farm stock (Non-Financial)
            </p>
          </div>
        </div>
          ...
          <Card className="border border-border shadow-sm">
            <CardHeader className="pb-3 border-b border-border">
              <CardTitle className="text-base font-semibold flex items-center space-x-2">
                ...
                <span>New Waste / Loss Entry</span>
              </CardTitle>
            </CardHeader>
            ...
            <CardHeader className="flex flex-row items-center justify-between py-4 px-6 border-b border-border">
              <div>
                <CardTitle className="text-base font-semibold text-foreground">
                  Recent Waste / Loss Records
                </CardTitle>
                <p className="text-xs text-muted-foreground">
                  Showing latest waste logs recorded ({total} total)
                </p>
              </div>
              <Button
                variant="ghost"

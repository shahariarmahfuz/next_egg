...
export default function FarmProductionPage() {
  ...
  const fetchHistory = useCallback(async () => {
    setLoading(true);
    try {
      const res = await farmService.getProductionHistory({
        page,
        size: pageSize,
      });
    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border pb-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-foreground">Record Farm Production</h1>
            <p className="text-xs sm:text-sm text-muted-foreground">
              Log daily egg collection and incoming trays to update farm stock
            </p>
          </div>
        </div>
            ...
            <CardHeader className="flex flex-row items-center justify-between py-4 px-6 border-b border-border">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Showing latest production batches logged ({total} total)
                </p>
              </div>
              <Button
                variant="ghost"
                ...
                <table className="w-full text-xs text-left">
                  <thead className="bg-muted/50 border-b border-border text-muted-foreground uppercase font-semibold">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-right">Units / Tray</th>
                      <th className="px-4 py-3 text-right text-emerald-600">Total Quantity</th>
                    ...
                    {loading ? (
                      ...
                      <tr>
                        <td colSpan={7} className="p-8 text-center text-muted-foreground">
                          No production entries recorded yet. Use the form on the left to record production.
                        </td>
                      </tr>
                    ) : (
                      history.map((item) => (
                        <tr key={item.id} className="hover:bg-muted/30 transition-colors">
                          ...
                          <td className="px-4 py-3 text-right text-muted-foreground">
                            {item.units_per_tray ? `${item.units_per_tray}` : "-"}
                          </td>

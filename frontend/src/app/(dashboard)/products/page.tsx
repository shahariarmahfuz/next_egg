...
export default function ProductsPage() {
  ...
  const { data: productsData, isLoading } = useQuery({
    queryKey: ["products", page, debouncedSearch, selectedCategory, selectedStatus, selectedProductType],
    queryFn: () =>
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-muted/50 border-b font-semibold text-muted-foreground uppercase text-[11px] tracking-wider">
              <tr>
                ...
                <th className="px-3 py-2.5 align-middle whitespace-nowrap">Opening Cost</th>
                    ...
                    <tr key={product.id} className="hover:bg-accent/40 transition-colors h-10">
                      ...
                      <td className="px-3 py-2 align-middle whitespace-nowrap">
                        {product.product_type === "FARM" ? (
                          <Badge variant="outline" className="text-[10px] py-0 px-1.5 h-5 bg-amber-500/10 text-amber-600 border-amber-500/30">Farm</Badge>
                        ) : (
                      ...
                      <td className="px-3 py-2 align-middle text-xs whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <span className={`font-bold ${product.current_stock > 0 ? "text-emerald-500" : "text-destructive"}`}>
                            {product.current_stock}
                          </span>
                          {product.current_stock <= product.minimum_stock && (
                            <span className="flex h-1.5 w-1.5 rounded-full bg-destructive animate-pulse" title="Low Stock">
                      ...
                      <td className="px-3 py-2 align-middle text-xs font-semibold text-foreground whitespace-nowrap">
                        {product.product_type === "FARM" ? <span className="text-muted-foreground text-[10px]">N/A</span> : formatCurrency(product.opening_stock_unit_cost)}
                      </td>
                      <td className="px-3 py-2 align-middle text-xs font-semibold text-primary whitespace-nowrap">
                        {product.product_type === "FARM" ? <span className="text-muted-foreground text-[10px]">N/A</span> : formatCurrency(product.selling_price)}
                      </td>
                      <td className="px-3 py-2 align-middle whitespace-nowrap">
                        <Badge

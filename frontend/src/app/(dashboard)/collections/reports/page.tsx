...
export default function CollectionsReportPage() {
  ...
  const [viewingCollection, setViewingCollection] = useState<CustomerCollectionItem | null>(null);

  const debouncedSearch = useDebounce(search, 300);

  const { data: collectionsData, isLoading } = useQuery({
    queryKey: ["collections-reports", page, debouncedSearch, paymentMethod, startDate, endDate],
    queryFn: () =>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <PageHeader title="Customer Collections Report" description="Comprehensive report of all customer collections." />
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={7} className="p-8 text-center text-muted-foreground">
                    No collection vouchers found matching your criteria.
                  </td>
                </tr>
              ) : (
                collections.map((collection, index) => {
                  ...
                  return (
                    <tr key={collection.id} className="hover:bg-accent/40 transition-colors h-10">
                      <td className="px-3 py-2 align-middle text-center font-medium text-muted-foreground whitespace-nowrap">{serialNumber}</td>
                      <td className="px-3 py-2 align-middle font-medium text-primary whitespace-nowrap">{collection.collection_no || '-'}</td>
                      <td className="px-3 py-2 align-middle text-muted-foreground whitespace-nowrap">{formatDate(collection.collection_date)}</td>
                      <td className="px-3 py-2 align-middle font-medium text-foreground whitespace-nowrap max-w-[200px] truncate" title={collection.customer?.name}>{collection.customer?.name}</td>
                      <td className="px-3 py-2 align-middle font-semibold text-emerald-500 whitespace-nowrap">{formatCurrency(collection.amount)}</td>
                      <td className="px-3 py-2 align-middle whitespace-nowrap">{getMethodBadge(collection.payment_method)}</td>
                      <td className="px-3 py-2 align-middle text-right whitespace-nowrap">
                        <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setViewingCollection(collection)} title="View Collection Details">
      ...
      {viewingCollection && (
        <CollectionViewModal collection={viewingCollection} isOpen={!!viewingCollection} onClose={() => setViewingCollection(null)} />
      )}
    </div>
  );
}

...
export default function ProductReturnsReportPage() {
  const today = new Date().toLocaleDateString('en-CA');
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState("");
  const [startDate, setStartDate] = useState(today);
  const [endDate, setEndDate] = useState(today);
  const [viewingReturn, setViewingReturn] = useState<ProductReturnItem | null>(null);

  const debouncedSearch = useDebounce(search, 300);

  const { data: returnsData, isLoading } = useQuery({
    queryKey: ["product-returns-reports", page, debouncedSearch, startDate, endDate],
    queryFn: () =>
  ...
  const returns: ProductReturnItem[] = returnsData?.data?.items || [];
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <PageHeader title="Product Returns Report" description="Comprehensive report of all product returns to suppliers." />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <Card className="glass-card border-blue-500/30 bg-blue-500/5">
          <CardContent className="p-4">
            <div className="text-xs font-semibold text-blue-600 uppercase tracking-wider mb-1">Total Return Amount</div>
          </CardContent>
        </Card>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={7} className="p-8 text-center text-muted-foreground">
                    No product returns found matching your criteria.
                  </td>
                </tr>
              ) : (
                returns.map((ret, index) => {
                  ...
                  return (
                    <tr key={ret.id} className="hover:bg-accent/40 transition-colors h-10">
                      ...
                      <td className="px-3 py-2 align-middle font-medium text-foreground whitespace-nowrap max-w-[200px] truncate" title={ret.supplier?.name || "Walk-in Vendor"}>{ret.supplier?.name || "Walk-in Vendor"}</td>

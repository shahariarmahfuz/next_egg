...
export default function ExpensesReportPage() {
  ...
  const { data: expensesData, isLoading } = useQuery({
    queryKey: ["expenses-reports", page, debouncedSearch, paymentMethod, startDate, endDate],
    queryFn: () =>
  ...
  const pageSize = 15;
  const aggregate = expensesData?.data?.aggregate || { total_amount: 0 };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <PageHeader title="Expense Report" description="Comprehensive report of all business expenses." />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <Card className="glass-card border-rose-500/30 bg-rose-500/5">
          <CardContent className="p-4">
            <div className="text-xs font-semibold text-rose-600 uppercase tracking-wider mb-1">Total Expense</div>
            <div className="text-xl font-extrabold text-rose-500">{formatCurrency(aggregate.total_amount)}</div>
          </CardContent>
        </Card>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/30 text-muted-foreground">
              <tr>
                ...
                <th className="px-3 py-2.5 align-middle font-medium text-left whitespace-nowrap">Category</th>
                ...
                <th className="px-3 py-2.5 align-middle font-medium text-left whitespace-nowrap">Description</th>
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                ...
                expenses.map((expense, index) => {
                  ...
                  return (
                    <tr key={expense.id} className="hover:bg-accent/40 transition-colors h-10">
                      <td className="px-3 py-2 align-middle text-center font-medium text-muted-foreground whitespace-nowrap">{serialNumber}</td>
                      <td className="px-3 py-2 align-middle text-muted-foreground whitespace-nowrap">{formatDate(expense.expense_date)}</td>
                      <td className="px-3 py-2 align-middle font-medium text-foreground whitespace-nowrap max-w-[200px] truncate">{expense.category_name}</td>
                      <td className="px-3 py-2 align-middle font-semibold text-rose-500 whitespace-nowrap">{formatCurrency(expense.amount)}</td>
                      <td className="px-3 py-2 align-middle whitespace-nowrap">{getMethodBadge(expense.payment_method)}</td>
                      <td className="px-3 py-2 align-middle font-medium text-foreground whitespace-nowrap max-w-[200px] truncate" title={expense.description}>{expense.description || "-"}</td>
